(function(window) {
	...
	var CalcEZ = document.querySelector('.calc-main'),
		// display things
		display = CalcEZ.querySelector('.calc-display span'),
		radDeg = CalcEZ.querySelector('.calc-rad'),
		smallerButton = CalcEZ.querySelector('.calc-smaller'),
		hold = CalcEZ.querySelector('.calc-hold'),
		lnButton = CalcEZ.querySelector('.calc-ln'),
		helpButton = CalcEZ.querySelector('.calc-info'),
		secondKeySet = [].slice.call(CalcEZ.querySelector('.calc-left').children, 12, 20),
		hiddenCopy = CalcEZ.querySelector('textarea'),
	...
	// colloect all keys...
	for (var k = 2; k--; ) {
		for (var l = CalcEZ.children[k + 1], m = l.children, n = m.length; n--; ) {
	...
	function getTargetKey(elm) {
		while (elm !== CalcEZ && elm.parentNode && elm.parentNode.style &&
	...
	function toggleCalc(doIt) {
		var cName = CalcEZ.className;

		if (doIt) {
			bigger = !bigger;
		}
		localStorage['bigger'] = bigger;
		CalcEZ.className = bigger ?
